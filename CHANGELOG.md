# dbt_zendesk v0.RELEASE.RELEASE
[PR #XX] introduces the following updates:
## 🚀 Feature Updates 🚀
- Added `solve_time_in_calendar_minutes` and `solve_time_in_business_minutes` to our `zendesk__ticket_metrics` model, which calculates calendar and business minutes the ticket was in the 'new', 'open', 'hold', or 'pending' status.
