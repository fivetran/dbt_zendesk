...
## Under the Hood
  - Introduced the `using_deliberate_quoting` variable (default `true`) to control whether column identifiers are quoted in staging tmp models. Set to `false` to use each warehouse's native default casing instead.
