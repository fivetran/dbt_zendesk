...
with reply_time_breach as (
  ...
  select * 
  from {{ ref('int_zendesk__reply_time_breach_combined') }}

), agent_work_calendar_breach as (

  select *
  from {{ ref('int_zendesk__agent_work_time_calendar_hours_breached') }}
...
{% if var('using_schedules', True) %}
  ...
  select *
  from {{ ref('int_zendesk__agent_work_time_business_hours_breached') }}
