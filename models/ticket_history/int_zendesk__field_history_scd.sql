...
-- we need to backfill to persist values that have been previously updated and are still valid 
    select 
        ...
        ,sum(case when {{ col.name }} is null 
                then 0 
                    ...
                    end) over ({{ fivetran_utils.partition_by_source_relation(package_name='zendesk', has_other_partitions='no') }} order by ticket_id, date_day rows unbounded preceding) as {{ col.name }}_field_partition
        {% endfor %}
    ...
    select
        ...
        ,first_value( {{ col.name }} ) over (partition by {{ col.name }}_field_partition, ticket_id {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }} order by valid_from asc rows between unbounded preceding and current row) as {{ col.name }}
