with ticket_historical_status as (
    ...
    select 
        ...
        first_value(valid_starting_at) over (partition by ticket_id {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }} order by valid_starting_at desc, ticket_id, source_relation rows unbounded preceding) as last_status_assignment_date,
        case when lag(status) over (partition by ticket_id {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }} order by valid_starting_at) = 'deleted' and status != 'deleted'
            then 1
            else 0
