...
with ticket_historical_status as (
    ...
    ({{ dbt.datediff(
            "cast(" ~ zendesk.week_start('ticket_status_crossed_with_schedule.status_schedule_start') ~ "as " ~ dbt.type_timestamp() ~ ")", 
    ...
    {{ zendesk.week_start('ticket_status_crossed_with_schedule.status_schedule_start') }} as start_week_date
