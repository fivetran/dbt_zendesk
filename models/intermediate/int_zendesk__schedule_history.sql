...
-- the formats for change_description vary, so it needs to be cleaned
    ...
    select 
        source_relation,
        schedule_id,
        rank() over (partition by schedule_id {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }} order by created_at desc) as schedule_id_index,
        created_at,
    ...
    select
        ...
        schedule_change,
        row_number() over (
            partition by schedule_id, valid_from {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }} -- valid from is type date
...
-- multiple changes can occur on one day, so we will keep only the latest change in a day.
    ...
    select
        ...
        lead(valid_from) over (
            partition by schedule_id {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }} order by schedule_id_index desc) as valid_until,
        schedule_change
