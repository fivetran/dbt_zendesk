...
-- Find and count all holidays that fall within a schedule range.
    ...
    select
        join_holidays.*,
        row_number() over (partition by schedule_id, start_time_utc, schedule_valid_from {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }} order by holiday_date, holiday_start_or_end) as valid_from_index,
        count(*) over (partition by schedule_id, start_time_utc, schedule_valid_from {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }}) as max_valid_from_index
    from join_holidays
    ...
    select
        ...
        case
            ...
            when holiday_start_or_end = '0_gap'
                then lag(holiday_ending_sunday) over (partition by schedule_id, start_time_utc, schedule_valid_from {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }} order by valid_from_index)
        ...
        case 
            when holiday_start_or_end = 'partition_start'
                then holiday_starting_sunday
            when holiday_start_or_end = '0_gap'
                then lead(holiday_starting_sunday) over (partition by schedule_id, start_time_utc, schedule_valid_from {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }} order by valid_from_index)
            when holiday_start_or_end = '1_holiday'
                then holiday_ending_sunday
            when holiday_start_or_end = 'partition_end'
...
-- Identifies whether a schedule overlaps with a holiday by comparing start and end times with holiday minutes.
    ...
    select 
        ...
        count(*) over (partition by schedule_id, valid_from, valid_until, start_time_utc, end_time_utc {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }}) as number_holidays_in_week
    from valid_minutes
...
-- Filter out records where holiday overlaps don't match, ensuring each schedule's holiday status is consistent.
    ...
    select 
        distinct *,
        cast(count(*) over (partition by schedule_id, valid_from, valid_until, start_time_utc, end_time_utc, holiday_name {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }})
