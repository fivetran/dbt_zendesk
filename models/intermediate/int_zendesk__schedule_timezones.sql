...
with split_timezones as (
    ...
    select 
        *,
        max(created_at) over (partition by schedule_id {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }}) as max_created_at
    ...
    select
        ...
        coalesce(case
            ...
            then max(case when schedule_id_index > 0 then valid_until end) 
                over (partition by schedule_id {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }})
            else valid_from
            end,
...
-- to identify effectively unchanged schedules.
    ...
    select 
        ...
        lag(schedule_valid_until) over (partition by schedule_id, start_time, end_time {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }} 
...
-- we want to maintain the intermediate schedule change.
    ...
    -- while non-adjacent schedules are treated as separate groups.
        sum(case when previous_valid_until = schedule_valid_from then 0 else 1 end) -- find if this row is adjacent to the previous row
            over (partition by schedule_id, start_time, end_time {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }} 
...
-- For each schedule_id, reset the earliest schedule_valid_from date to 1970-01-01 for full schedule coverage.
    ...
    select
        ...
        end_time,
        case 
            when schedule_valid_from = min(schedule_valid_from) over (partition by schedule_id {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }}) then '1970-01-01'
