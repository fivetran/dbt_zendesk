...
-- Split the change_description into "from" and "to" strings
    ...
    select
        ...
        min(created_at) over (partition by user_id {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }}) as min_created_at_per_user
    from audit_logs
...
-- Captures all subsequent "to" roles
    ...
    select
        ...
        lead(created_at) over (partition by user_id {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }} order by created_at asc) as valid_ending_at,
        to_role as role
    from split_to_from_deduped
