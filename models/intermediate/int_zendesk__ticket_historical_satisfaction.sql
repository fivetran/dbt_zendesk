with satisfaction_updates as (
    ...
    select
        source_relation,
        ticket_id,
        first_value(value) over (partition by ticket_id {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }} order by valid_starting_at desc, ticket_id rows unbounded preceding) as latest_satisfaction_reason
    ...
    select
        source_relation,
        ticket_id,
        first_value(value) over (partition by ticket_id {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }} order by valid_starting_at desc, ticket_id rows unbounded preceding) as latest_satisfaction_comment
    from satisfaction_updates
    ...
    select
        source_relation,
        ticket_id,
        first_value(value) over (partition by ticket_id {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }} order by valid_starting_at, ticket_id rows unbounded preceding) as first_satisfaction_score,
        first_value(value) over (partition by ticket_id {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }} order by valid_starting_at desc, ticket_id rows unbounded preceding) as latest_satisfaction_score
    from satisfaction_updates
    ...
    select
        source_relation,
        ticket_id,
        count(value) over (partition by ticket_id {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }}) as count_satisfaction_scores,
        case when lag(value) over (partition by ticket_id {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }} order by valid_starting_at desc) = 'good' and value = 'bad'
            then 1
            else 0
                end as good_to_bad_score,
        case when lag(value) over (partition by ticket_id {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }} order by valid_starting_at desc) = 'bad' and value = 'good'
            then 1
            else 0
