...
latest_ticket_form as (
    select
      *,
      row_number() over(partition by ticket_form_id {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }} order by updated_at desc) as latest_form_index
