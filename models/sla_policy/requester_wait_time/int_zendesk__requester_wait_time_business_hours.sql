...
-- cross schedules with work time
    ...
    select 
      ...
      ({{ dbt.datediff(
            "cast(" ~ zendesk.week_start('ticket_status_crossed_with_schedule.valid_starting_at') ~ "as " ~ dbt.type_timestamp() ~ ")", 
    ...
    {{ zendesk.week_start('ticket_status_crossed_with_schedule.valid_starting_at') }} as start_week_date
  ...
  select 
    *,
    {{ fivetran_utils.timestamp_add(
      ...
      "cast(" ~ zendesk.week_start('valid_starting_at') ~ " as " ~ dbt.type_timestamp() ~ " )"
      ) }} as sla_breach_at
