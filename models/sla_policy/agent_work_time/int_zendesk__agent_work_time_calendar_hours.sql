  ...
  select 
    *,
    ...
    sum({{ dbt.datediff(
            ...
            'valid_ending_at', 
            'minute') }} ) 
      over (partition by ticket_id, sla_applied_at, source_relation order by valid_starting_at rows between unbounded preceding and current row) as running_total_calendar_minutes
...
select 
  ...
  case when (target - running_total_calendar_minutes) < 0 
      and 
        ...
        (partition by ticket_id, sla_applied_at, source_relation order by valid_starting_at) >= 0 
        or 
        lag(target - running_total_calendar_minutes) over
        (partition by ticket_id, sla_applied_at, source_relation order by valid_starting_at) is null) 
