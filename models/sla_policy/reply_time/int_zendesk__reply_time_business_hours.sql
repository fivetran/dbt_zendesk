...
with ticket_schedules as (
  ...
  select
    ...
    ticket_sla_applied_with_schedules.priority_applied,
    ...
    *,
    ...
    case when (target - sum_lapsed_business_minutes) < 0 
      and 
        ...
        (partition by ticket_id, sla_policy_name, metric, sla_applied_at {{ partition_by_source_relation() }} order by week_number, schedule_start_time) >= 0 
        or 
        lag(target - sum_lapsed_business_minutes) over
        (partition by ticket_id, sla_policy_name, metric, sla_applied_at {{ partition_by_source_relation() }} order by week_number, schedule_start_time) is null) 
  ...
  select
    ...
    max(case when is_breached_during_schedule then sla_breach_at else null end) over (partition by ticket_id, sla_policy_name, metric, sla_applied_at, target {{ partition_by_source_relation() }}) as sla_breach_exact_time,
    week_number
