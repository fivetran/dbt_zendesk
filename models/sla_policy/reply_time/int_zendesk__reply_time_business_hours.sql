...
with ticket_schedules as (
  ...
  select 
    ...
    ({{ dbt.datediff(
            "cast(" ~ zendesk.week_start('sla_policy_applied.sla_applied_at') ~ "as " ~ dbt.type_timestamp() ~ ")", 
    ...
    {{ zendesk.week_start('sla_policy_applied.sla_applied_at') }} as start_week_date
  ...
  select
    *,
    ...
    {{ zendesk.week_start('sla_applied_at') }} as starting_point,
    {{ fivetran_utils.timestamp_add(
        "minute",
        "cast(((7*24*60) * week_number) + (schedule_end_time + remaining_minutes) as " ~ dbt.type_int() ~ " )",
        "cast(" ~ zendesk.week_start('sla_applied_at') ~ " as " ~ dbt.type_timestamp() ~ ")" ) }} as sla_breach_at,
    {{ fivetran_utils.timestamp_add(
        "minute",
        "cast(((7*24*60) * week_number) + (schedule_start_time) as " ~ dbt.type_int() ~ " )",
        "cast(" ~ zendesk.week_start('sla_applied_at') ~ " as " ~ dbt.type_timestamp() ~ ")" ) }} as sla_schedule_start_at,
    {{ fivetran_utils.timestamp_add(
        "minute",
