...
select 
    *,
    row_number() over (partition by sla_policy_id, metric, priority {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }} order by sla_policy_updated_at desc) = 1 as is_most_recent_record
from final