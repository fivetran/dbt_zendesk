...
with timezone as (
    ...
    select 
        *,
        ...
        lag(daylight_end_utc, 1) over (partition by time_zone {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }} order by daylight_end_utc asc) as last_daylight_end_utc,
        ...
        lead(daylight_start_utc, 1) over (partition by time_zone {{ fivetran_utils.partition_by_source_relation(package_name='zendesk') }} order by daylight_start_utc asc) as next_daylight_start_utc
