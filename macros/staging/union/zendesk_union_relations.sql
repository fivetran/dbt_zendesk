...
{%- macro zendesk_union_relations(relations, aliases=none, column_override=none, include=[], exclude=[], source_column_name='_dbt_source_relation', where=none) -%}
    {{ return(adapter.dispatch('zendesk_union_relations', 'zendesk')(relations, aliases, column_override, include, exclude, source_column_name, where)) }}
{% endmacro %}
        ...
        (
            select
                ...
                {% for col_name in ordered_column_names -%}
                    ...
                    cast({{ col_name }} as {{ col_type }}) as {{ col.quoted if var('using_deliberate_quoting', True) else (adapter.quote(col.column | upper) if target.type == 'snowflake' else col.column) }} {% if not loop.last %},{% endif -%}
